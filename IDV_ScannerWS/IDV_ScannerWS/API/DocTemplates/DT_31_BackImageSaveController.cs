...
namespace IDV_ScannerWS.API.DocTemplates
{
    ...
    public class DocumentRelationInfoBI
    {
    ...
    public class DT_31_BackImageSaveController : ApiController
    {
        //--------------------------------------------------------------------------
        ...
        public string Post(string W, string H)
        {
            ...
            try
            {
                W = W.Trim();
                H = H.Trim();
                DocumentRelationInfoBI DBI = new DocumentRelationInfoBI();
                ...
                if (DT.Rows != null)
                {
                    if (DT.Rows.Count == 1)
                    {
                        SQ.Execute_TSql("Delete From Template_07_Images Where (CID = '" + DBI.CountryID + "') And (SID = '" + DBI.StateID + "') And (DTID = '" + DBI.DocumentTypeID + "') And (DID = '" + DBI.DocumentID + "') And (ImageID = '2')");
                        ...
                        foreach (string file in httpRequest.Files)
                        {
                            ...
                            if (postedFile != null && postedFile.ContentLength > 0)
                            {
                                ...
                                SqlCom.Parameters.Add(new SqlParameter("@ImageID", "2"));
